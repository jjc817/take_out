...
//    }
    ...
    @ApiOperation("订单信息")
    @RequiresPermissions("takeout:orderdetail:detailInfo")
    public Result<List<OrderDetailEntity>> getById(@PathVariable("id") Long id){

        List<OrderDetailEntity> voList = orderDetailService.getByOrderId(id);

        return new Result<List<OrderDetailEntity>>().ok(voList);
