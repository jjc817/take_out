				...
				<view class="divContent">
					...
					<view class="divOrders" v-if="flag && user">
						...
						<view class="dishImage">
								...
								<u-image v-for="(item,index) in order[0].orderDetails.slice(0,4)" :key="index" 
						...
						<view class="result">
							<text>共{{order[0].sumNum}} 件商品，优惠</text>
							<text class="price" v-if="order[0].discount!=null">￥{{order[0].discount}} </text>
							<text v-else>￥0</text>
							实付，<text class="price">￥{{order[0].amount}}</text>
						</view>
<!-- 						<view class="btn" v-if="order[0].status === 4">
							<view class="btnAgain" @click="addOrderAgain(order[0])">再来一单</view>
						</view> -->
						<view class="btn">
							<view class="btnAgain" @click="addOrderAgain(order[0])">再来一单</view>
							<view class="btnAgain" @click="toOrderDetail(order[0])">查看详情</view>
						</view>
					</view>
