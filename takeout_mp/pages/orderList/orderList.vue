				...
				<u-list @scrolltolower="scrolltolower" lowerThreshold="150">
					<block v-for="(order,index) in orderList" :key="index">
						...
						<view class="item">
							...
							<view class="dishList">
								<view v-for="(item,i) in order.orderDetails.slice(0,4)" :key="index" class="item">
									<text>{{item.name}}</text>
									<text>x{{item.number}}</text>
							
								</view>
							</view>
							<view class="dishImage">
								
									<u-image v-for="(item,i) in order.orderDetails.slice(0,4)" :key="index" 
									class="image" width="172rpx" height="172rpx" :src="imgPathConvert(item.image)">
							...
							<view class="result">
								...
								<text>共{{order.sumNum}} 件商品，优惠</text>
								<text class="price" v-if="order.discount!=null">￥{{order.discount}}</text>
								<text v-else>￥0</text>
								，实付<text class="price">￥{{order.amount}}</text>
							</view>
					<!-- 		<view class="btn" v-if="order.status === 4">
								<view class="btnAgain" @click="addOrderAgain(order)">再来一单</view>
								<view class="btnAgain" @click="toOrderDetail(order)">查看详情</view>
							</view> -->
							<view class="btn">
								<view class="btnInfo" @click="toOrderDetail(order)">查看详情</view>
							</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			imgPathConvert(path) {
				return getApp().globalData.QiNiuYunUrl + path
				// return imgPath(path)
			},
			getStatus(status) {
				let str = ''
