...
import io.renren.common.dao.AddressBookDao;
...
//获得当前用户id
        ...
        List<OrderDetailEntity> orderDetails = shoppingCarts.stream().map((item) -> {
            ...
            amount.addAndGet(item.getAmount().multiply(BigDecimal.valueOf(item.getNumber())).doubleValue());
            return orderDetail;
        }).collect(Collectors.toList());

        BigDecimal discount = ordersDTO.getDiscount();
        ...
        ordersDTO.setAmount(BigDecimal.valueOf(amount.get()).subtract(discount));
...
//        }
    ...
    @Override
    public void again(Long id,Long userId) {

        OrdersEntity ordersEntity = ordersDao.selectById(id);
        //获取该订单对应的所有的订单明细表
        List<OrderDetailEntity> orderDetailList = orderDetailService.getByOrderId(id);
        ...
        List<ShoppingCartEntity> shoppingCartList = orderDetailList.stream().map((item) -> {
            ...
            shoppingCart.setAmount(dishDTO.getPrice().divide(BigDecimal.valueOf(100)));//数据库存的不含小数点，要除100
    ...
    @Override
    public List<OrdersDTO> getOrderById(Long id) {
        List<OrdersDTO> list = new ArrayList<>();
        OrderDetailEntity orderDetailEntity = orderDetailService.selectById(id);

        OrdersEntity ordersEntity = selectById(id);
        List<OrdersDTO> ordersDtoList = new ArrayList<>();
        OrdersDTO ordersDto = new OrdersDTO();
        BeanUtils.copyProperties(ordersEntity, ordersDto);
        Long userId = ordersEntity.getUserId();
        LambdaQueryWrapper<OrderDetailEntity> queryWrapper1 = new LambdaQueryWrapper<>();
        queryWrapper1.eq(OrderDetailEntity::getOrderId, ordersEntity.getId());
        List<OrderDetailEntity> orderDetails = orderDetailDao.selectList(queryWrapper1);
        ordersDto.setOrderDetails(orderDetails);
